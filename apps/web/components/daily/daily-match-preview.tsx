    ...
    >
      <GlassPanel className="p-4">
        <div className="flex items-center justify-between mb-3">
          <span className="text-[10px] uppercase tracking-[0.2em] text-white/25">{match.stage}</span>
          {isLive && (
          ...
          {isFinished && (
            <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-white/35">FT</span>
          )}
          {!isLive && !isFinished && (
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-lg">{home ? getFlag(home.code) : "🏳️"}</span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-lg">{away ? getFlag(away.code) : "🏳️"}</span>
        ...
        {match.venue && (
          <p className="mt-3 text-[10px] text-white/15 uppercase tracking-wider">{match.venue}</p>
        )}
      </GlassPanel>
    </motion.div>
  );
